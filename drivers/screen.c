...
void print_char(char character, int col, int row, char attribute_byte) {
    ...
    // so that it will be advanced to the first col of the next row
    if (character == '\n') {
    ...
    // Make scrolling adjustment, for when we reach the bottom of
    // the screen.
...
int handle_scrolling(int cursor_offset) {
    ...
    last_line = (char*)(get_screen_offset(0, MAX_ROWS - 1) + VIDEO_ADDRESS);
